...
from propicker.paths import PROPICKER_MODEL_FILE, DEEPETPICKER_PROPICKER_DIR
...
for line in lines:
    with open(pre_config_file, "a") as f:
        f.write(line + "\n")
os.system(f"python {DEEPETPICKER_PROPICKER_DIR}/bin/preprocess.py --pre_configs {pre_config_file}")

# generate DeepETPicker training config
os.system(
    f"python {DEEPETPICKER_PROPICKER_DIR}/bin/generate_train_config.py \
